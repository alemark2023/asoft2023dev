...
export default {
    ...
    methods: {
        ...
        clickReportCustomer(row) {
            const query = `apply_conversion_to_pen=false&brand_id&category_id=&date_end&date_start&document_type_id=80&item_id&month_end&month_start&period&person_id=${row.id}&type=sale&type_person=customers&user&user_id=&user_type=&web_platform_id&history=true&name=${row.name}&document_type=${row.document_type}&number=${row.number}`;
