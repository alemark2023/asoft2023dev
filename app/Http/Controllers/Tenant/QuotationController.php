...
//    }
    ...
    public function recordOther($id)
    {
        $record = Quotation::query()->with('items', 'items.relation_item')->findOrFail($id);
        ...
        return [
            'form' => [
                ...
                'customer_address_id' => $record->customer_address_id,
                'account_number' => $record->account_number,
                'phone' => $record->phone,
                'contact' => $record->contact,
                'description' => $record->description,
                'shipping_address' => $record->shipping_address,
                'referential_information' => $record->referential_information,
            ],
            'store_items' => $this->getItems($record->items)
        ];
    }
    ...
    private function getItems($items)
    {
        return $items->transform(function ($row) {
            return [
                ...
                'unit_value' => floatval($row->unit_value),
                'has_igv' => $row->item->has_igv,
                'has_isc' => $row->item->has_isc,
                'has_icbper' => $row->item->has_icbper,
    ...
    public function store(QuotationRequest $request)
    {
        ...
        try {
            ...
            $record = Quotation::query()->firstOrNew(['id' => $id]);
            $record->fill($request->all());

            $record->customer = PersonInput::set($customer_id, $customer_address_id);
            $record->establishment = EstablishmentInput::set($establishment_id);
            $record->terms_condition = $this->getTermsCondition();
            $record->save();
            ...
            $this->savePayments($record, $payments);

//            $record->filename = join('-', [$record->prefix, $record->id, date('Ymd')]);
//            $record->save();

            $this->createPdf($record, "a4", $record->filename);
