    ...
    public function recordsSales(Request $request)
    {
        ...
        $sale_notes = $sale_notes->join('sale_notes', 'sale_note_items.sale_note_id', '=', 'sale_notes.id')
                                    ->select(DB::raw("sale_note_items.id as id, sale_notes.prefix as series, sale_notes.id as number,
                                            sale_note_items.unit_price as price, sale_notes.date_of_issue as date_of_issue, sale_notes.customer->'$.name' as name"))
        ...
        $documents = DocumentItem::where('item_id', $form->item_id)
                                    ...
                                    ->select(DB::raw("document_items.id as id, documents.series as series, documents.number as number,
                                            document_items.unit_price as price, documents.date_of_issue as date_of_issue, documents.customer->'$.name' as name"))
                                    ->orderBy('created_at', 'desc');
