                ...
                @for ($i = 0; $i < count($clear_type); $i++)
                    ...
                    <table class="">
                        ...
                        <tr style="background-color: #ebebeb;">
                            ...
                            <th style="padding: 5px; text-align: center;">Usuario/Vendedor</th>
                            ...
                            <th style="padding: 5px; text-align: center;" class="">Moneda</th>
                        ...
                        @for ($t = 0; $t < count($records); $t++)
                            ...
                            <tr>
                                <td style="padding: 5px; text-align: center;">{{$t+1}}</td>
                                ...
                                <td style="padding: 5px; text-align: center;">{{$document_type->id}}</td>
                                <td style="padding: 5px; text-align: center;">{{$value->series}}-{{$value->number}}</td>
                                <td style="padding: 5px; text-align: center;">{{$value->date_of_issue->format('Y-m-d')}}</td>
                                <td style="padding: 5px; text-align: center;">{{isset($value->invoice) ? $value->invoice->date_of_due->format('Y-m-d'):''}}</td>
                                ...
                                @endif
                                <td style="padding: 5px; text-align: center;">{{$serie_affec }} </td>
                                @if ($columns->guides->visible)
                                <td style="padding: 5px; text-align: center;">
                                ...
                                <td style="padding: 5px; text-align: center;">{{ ($value->quotation) ? $value->quotation->number_full : '' }}</td>
                                <td style="padding: 5px; text-align: center;">{{ isset($value->quotation->sale_opportunity) ? $value->quotation->sale_opportunity->number_full : '' }}</td>
        
                                <?php $stablihsment = \App\CoreFacturalo\Helpers\Template\ReportHelper::getLocationData($value); ?>
                                @if ($columns->district->visible)
                                <td style="padding: 5px; text-align: center;">{{$stablihsment['district']}}</td>
                                @endif
                                @if ($columns->department->visible)
                                <td style="padding: 5px; text-align: center;">{{$stablihsment['department']}}</td>
                                @endif
                                @if ($columns->province->visible)
                                <td style="padding: 5px; text-align: center;">{{$stablihsment['province']}}</td>
                                @endif
                                @if ($columns->client_direction->visible)
                                <td style="padding: 5px; text-align: center;">{{$value->customer->address}}</td>
                                @endif
                                <td style="padding: 5px; text-align: center;">{{$value->customer->name}}</td>
                                @if ($columns->ruc->visible)
                                <td style="padding: 5px; text-align: center;">{{$value->customer->number}}</td>
                                @endif
                                <td style="padding: 5px; text-align: center;">{{$value->state_type->description}}</td>
                                ...
                                <td style="padding: 5px; text-align: center;">{{$value->currency_type_id}}</td>
                                @if ($columns->web_platforms->visible)
                                <td style="padding: 5px; text-align: center;">
                                ...
                                <td style="padding: 5px; text-align: center;">{{$value->purchase_order}}</td>
        
                                @if($value->sale_note)
                                    @if ($columns->note_sale->visible)
                                    <td style="padding: 5px; text-align: center;">{{ $value->sale_note->number_full }}</td>
                                    @endif
                                    @if ($columns->date_note->visible)
                                    <td style="padding: 5px; text-align: center;">{{ $value->sale_note->date_of_issue->format('Y-m-d') }}</td>
                                    @endif
                                @else
                                    @if ($columns->note_sale->visible)
                                ...
                                @if($signal == '07')
                                    ...
                                    @else
                                        @if ($columns->total_charge->visible)
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_charge}}</td>
                                        @endif
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_exonerated}}</td>
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_unaffected}}</td>
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_free}}</td>
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_taxed}}</td>
                                        <td style="padding: 5px; text-align: center;">{{$value->total_discount}}</td>
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_igv}}</td>
                                        @if ($columns->total_isc->visible)
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total_isc}}</td>
                                        @endif
                                        <td style="padding: 5px; text-align: center;">{{$signal == '07' ? "-" : ""  }}{{$value->total}}</td>
                                    @endif
        
                                @else
                                    @if ($columns->total_charge->visible)
                                        <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_charge}}</td>
                                    @endif
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_exonerated}}</td>
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_unaffected}}</td>
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_free}}</td>
        
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_taxed}}</td>
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_discount}}</td>
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_igv}}</td>
                                    @if ($columns->total_isc->visible)
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total_isc}}</td>
                                    @endif
                                    <td style="padding: 5px; text-align: center;">{{ (in_array($document_type->id,['01','03']) && in_array($value->state_type_id,['09','11'])) ? 0 : $value->total}}</td>
