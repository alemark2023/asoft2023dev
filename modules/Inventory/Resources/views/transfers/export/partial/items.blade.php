...
<table class="full-width">
    <thead>
    <tr>
        <th class="ten-width">ITEM</th>
        <th class="ten-width">CODIGO INTERNO</th>
        <th  class="fourteen-width">DESCRIPCIÓN PRODUCTO</th>
    ...
    @foreach ($inventories as $index => $inventory)
        ...
        */
        ...
        <tr>
            <td>{{$index + 1}}</td>
            <td>{{$itemCollection['internal_id']}}</td>
            <td>{{$itemCollection['description']}}</td>
            <td>{{$itemCollection['unit_type_text']}}</td>
            <td>{{$qty}}</td>
            <td>{{$lot_code}}</td>
            <!--            <td>SERIE</td>-->
        </tr>
