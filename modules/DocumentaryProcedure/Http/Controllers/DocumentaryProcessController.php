    ...
    class DocumentaryProcessController extends Controller {
        public function index() {
            ...
            if (request()->ajax()) {
                $filter = request('name');
                if ($filter) {
                    $processes->where('name', 'like', "%$filter%");
                }
        ...
        public function store(ProcessRequest $request) {
            $process = DocumentaryProcess::create($request->only('name', 'description', 'active','price'));
        ...
        public function update(ProcessRequest $request, $id) {
            ...
            $process->fill($request->only('name', 'description', 'active', 'price'));
            $process->save();
